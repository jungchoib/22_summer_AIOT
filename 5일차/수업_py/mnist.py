...
task = kpu.load(0x300000)			#load model from flash address 0x200000 모델 불러오기
a=kpu.set_outputs(task, 0, 10,1,1)  # 모델의 output은 1X1이 10개
sensor.run(1)
while True:
    img = sensor.snapshot()
    ...
    a=img2.pix_to_ai();				#generate data for ai   ai데이터 생성
    ...
    lcd.draw_string(224,0,"%d: %.3f"%(max_index,pmax),lcd.WHITE,lcd.BLACK)  # 아규max?